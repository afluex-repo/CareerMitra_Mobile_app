...
import afluex.parent.careermitra.app.AppConfig;
...
public class ServiceGenerator {
    ...
    private static Retrofit.Builder builderFile = new Retrofit.Builder().baseUrl(BuildConfig.BASE_URL_IMAGE).addConverterFactory(GsonConverterFactory.create(gson));
    ...
    public static <S> S createServiceRecharge(Class<S> serviceClass) {
        return createServiceRecharge(serviceClass, null, null);
    }

    private static <S> S createServiceRecharge(Class<S> serviceClass, String username, String password) {
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(BuildConfig.RECHARGE_URL)
                .client(provideOkHttpClient())
                .addConverterFactory(ScalarsConverterFactory.create()).addConverterFactory(GsonConverterFactory.create(gson))
                .build();
        return retrofit.create(serviceClass);
...
//            String authToken = Credentials.basic(BuildConfig.M2P_USERNAME, BuildConfig.M2P_PASSWORD);
    ...
    private static <S> S createServiceUtilityV2(Class<S> serviceClass, final String authToken) {
